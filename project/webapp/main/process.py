...
from cams.models import Detections, Webcam
from django.core.files.base import ContentFile

warnings.filterwarnings("ignore", category=RuntimeWarning)

dir_path = os.path.dirname(os.path.realpath(__file__))
protoFile = dir_path + "/pose/coco/pose_deploy_linevec.prototxt"
weightsFile = dir_path + "/pose/coco/pose_iter_440000.caffemodel"
nPoints = 18
...
def run(input_source, device, callback, resolution=360):
    ...
    while cv2.waitKey(1) < 0:
        ...
        if check(last, detected_keypoints):
            ...
            cv2.putText(frameClone,
                        format(datetime.datetime.now().isoformat()) + " Драка!", (50, 50),
                        cv2.FONT_HERSHEY_COMPLEX,
                        .8,
                        (0, 0, 255), 2, lineType=cv2.LINE_AA)

            ref, buf = cv2.imencode('.jpg', frameClone)
            content = ContentFile(buf.tobytes())
            detected = Detections(cam_name=Webcam.objects.filter(link=input_source).values('title')
                                  , cam_link=Webcam.objects.filter(link=input_source).values('id'))

            detected.detected_frame.save('output.jpg', content)
            callback(frameClone)
            ...
            cv2.putText(frameClone, (datetime.datetime.now().isoformat()),
                        (5, 25),
                        cv2.FONT_HERSHEY_COMPLEX,
                        .8,
                        (0, 150, 255), 2, lineType=cv2.LINE_AA)
